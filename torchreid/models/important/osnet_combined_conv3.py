...
class OSNet(nn.Module):
    ...
    def featuremaps2(self, x):
        x = self.conv1(x)
        x = self.maxpool(x)
        x = self.conv2(x)
        x = self.conv3(x)  # Arrêt après conv3

        # Extraction des features de Harris après conv3
        ...
        # Sélection d'une partie des canaux de conv3 (par exemple, la moitié)
    ...
    def extract_harris_features(self, x):
        """Extract Harris corner features from the input image batch."""
        harris_features = []
        ...
        for img in x:  # Parcours chaque image du batch
            ...
            # Normalisation et transformation en tenseur PyTorch
