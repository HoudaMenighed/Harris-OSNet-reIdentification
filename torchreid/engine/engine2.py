...
class Engine(object):
    ...
    def _evaluate(
        ...
        distmat = metrics.compute_distance_matrix(qf, gf, dist_metric).numpy()

        if rerank:
            print('Applying person re-ranking ...')
            distmat_qq = metrics.compute_distance_matrix(qf, qf, dist_metric)
            distmat_gg = metrics.compute_distance_matrix(gf, gf, dist_metric)
            distmat = re_ranking(distmat, distmat_qq.numpy(), distmat_gg.numpy())
        ...
        # Save CMC curve
        plt.figure()
        plt.plot(ranks, cmc[: len(ranks)], marker='o', linestyle='-', label='CMC Curve')
        plt.xlabel('Rank')
        ...
        # Save mAP progression curve
        plt.figure()
        plt.plot(range(1, len(cmc) + 1), cmc, marker='s', linestyle='--', label='mAP')
        plt.xlabel('Rank')
        plt.ylabel('mAP')
        plt.title('mAP Curve')
        plt.legend()
        plt.grid(True)
        map_path = os.path.join(save_dir, 'map_curve.png')
