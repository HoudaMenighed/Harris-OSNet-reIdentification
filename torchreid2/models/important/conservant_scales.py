...
class OSNet(nn.Module):
    ...
    def featuremaps(self, x):
        ...
        # harris_features : [batch_size, num_scales, H, W]
        ...
        if x.size(2) != harris_features.size(2) or x.size(3) != harris_features.size(3):
            harris_features = F.interpolate(
                ...
                mode='bilinear',
                align_corners=False
            )  # harris_features : [batch_size, num_scales, conv3_H, conv3_W]

        # Déterminer combien de canaux remplacer
        num_channels = x.size(1)  # Nombre total de canaux de conv3
        num_harris_channels = harris_features.size(1)  # Nombre d'échelles utilisées
        num_remaining_channels = num_channels - num_harris_channels  # Canaux conservés de conv3

        # Séparer les canaux de conv3
    ...
    def extract_harris_features(self, x, scales=[1.0, 0.75, 0.5]):
        ...
        for img in x:  # Parcours chaque image du batch
            ...
            multi_scale_features = torch.stack(multi_scale_features, dim=0)
